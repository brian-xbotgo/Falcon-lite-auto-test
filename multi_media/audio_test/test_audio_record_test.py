...
def test_audio_record(device_serial: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if not success:
            ...
            return False, f"文件分析失败: {mp4info_output}"
        ...
        for line in mp4info_output.split('\n'):
            line = line.strip()
            if line.startswith('2') and 'audio' in line:
                audio_track_found = True
                audio_info = line
                log.info(f"音频轨道信息: {audio_info}")
                
                # 提取kbps数值
                kbps_match = re.search(r'(\d+)\s*kbps', audio_info)
                if kbps_match:
                    audio_kbps = int(kbps_match.group(1))
                    log.debug(f"音频码率: {audio_kbps} kbps")
                break
        ...
        # 判断测试结果
        if audio_track_found and audio_kbps > 2:
            log.info("音频测试通过")
            # ADBService.exec_shell(device_serial, f"rm -rf {video_path}")
            return True, f"音频录制测试通过, {audio_info}"
        else:
            ...
            if audio_kbps <= 2:
                error_msg.append(f"音频码率过低({audio_kbps} kbps)")
            error_detail = "、".join(error_msg)
            log.error(f"音频验证失败: {error_detail}")
            log.error(f"完整输出:\n{mp4info_output}")
            return False, f"音频测试不通过；{audio_info}"
