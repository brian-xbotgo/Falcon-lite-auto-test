...
"""
测试用例：2K30fps录制测试
功能：自动测试视频录制功能并验证录制文件完整性（2K@30fps）
作者：wuzhibin
创建时间：2026-05-08
"""
import os
...
def test_2K30fps_record(device_serial: str) -> tuple[bool, str]:
    """
    测试用例A：2K30fps录制测试
    :param device_serial: 设备序列号
    :return: (测试结果:True/False, 测试消息/备注)
    """
    log.info("开始执行2K30fps录制自动化测试")
    ...
    try:
        ...
        if not success:
            log.error(f"推送set_2K30fps.bin失败: {remote_path}")
            tools_ready = False
        ...
        # 发送帧率配置
        success, _ = ADBService.exec_shell(device_serial, "mosquitto_pub -t 'FHR' -f /tmp/set_2K30fps.bin")
        if not success:
        ...
        if video_track_found and audio_track_found and resolution_found and fps_found:
            log.info("2K30fps视频文件验证通过")
            return True, f"2K30fps录制测试通过, {video_info}"
        else:
            error_msg = []
            ...
            if not resolution_found:
                error_msg.append("分辨率非2560x1440")
            if not fps_found:
                error_msg.append("帧率非30fps")
            error_detail = "、".join(error_msg)
            log.error(f"视频文件验证失败: {error_detail}")
            log.error(f"完整输出:\n{mp4info_output}")
            return False, f"2K30fps验证失败: {error_detail}, 输出: {mp4info_output[:200]}..."
