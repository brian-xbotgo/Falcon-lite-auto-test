...
# 一、开源版项目标准目录架构（清理冗余+明确用途）
**已删除旧的 `service/`、`model/`、`utils/`、根目录engine目录**，所有内容已迁移至新架构，最终结构如下：
**版本更新：V2.4（2026-04-28 开源发布版）- 完整README、多语言支持、设备端时间验证优化**
```
...
- **`test_case_number` 字段**：字符串类型业务编号，支持初始化留空，后续人工维护
- 执行顺序自动保证：先A后B，同类按编号排序
- 无需手动排序，引擎自动处理
- 注册模块、测试报告确认页面及最终测试报告中均新增该展示列
...
## ✅ 人工测试复位命令支持
人工测试用例支持自动复位功能，返回值格式：
```python
# 普通人工测试：返回 (状态, 提示信息)
return None, "请观察设备状态"

# 带复位命令的人工测试：返回 (状态, 提示信息, 复位命令)
return None, "请观察电机运动", reset_cmd
```
- 测试引擎在人工确认通过后**自动执行复位命令**
- 无需在测试函数中手动执行，也无需修改引擎代码
- 适合需要执行测试命令后等待观察，确认后自动复位的场景

## ✅ 十六进制命令编写规范
所有包含`\x`十六进制转义的字符串必须使用**原始字符串**（r前缀）：
```python
# ✅ 正确写法：使用r前缀，转义序列完整传递到shell
test_cmd = r'''printf '\x00\x66\x20\x03\x00\x06' | mosquitto_pub -h localhost -t "A" -s'''

# ❌ 错误写法：Python会提前解析\x，导致转义丢失
test_cmd = '''printf '\x00\x66\x20\x03\x00\x06' | mosquitto_pub -h localhost -t "A" -s'''
```
- 该规范适用于所有printf/echo命令
- 引擎会自动处理命令转义，防止Windows shell解析特殊字符
