# RV1126B ADB 自动化冒烟测试工具 - 开源版项目设计与架构
**版本**：V2.4（2026-04-28 开源发布版）
**核心原则**：分层解耦、高内聚低耦合、USB有线ADB专属、日志/报告统一归档、单一模块修改无交叉、测试用例模块化分离
**✅ 已完成开源准备：完整README、多语言支持、设备端时间验证优化**
...
8. ✅ 设备文件管理器（上传/下载/预览）
9. ✅ 设备日志一键打包下载
10. ✅ 多媒体自动化测试（录制验证、打点功能、mp4info分析）
...
│   ├── tab_device_manager.py # 设备文件管理器UI
│   └── tab_file_manager.py # 本地文件管理页面UI
├── commons/                # ✅ 微内核核心（稳定不变）
...
| `engine/test_service.py` | 测试用例调度、自动发现注册、人工确认管理 |
| `engine/report_service.py` | 测试报告生成、保存、预览确认 |
...
- Qt跨线程日志导致进程静默退出
- Bleak多实例并发创建崩溃
- QTableWidget DPI缩放重绘残留
- Qt deleteLater() 悬空指针RuntimeError
- 递归测试调度栈溢出
- BLE扫描重复点击保护
- asyncio事件循环内存泄漏
- 报告服务语法错误与导入问题
